...
class NumCat():
    def __init__(self, num, weight=None):
        ...
        self.bias = 0
        self.negative_weight_multiplier = 1
        self.positive_weight_multiplier = 1
        if weight == None:
    ...
    def get_normalised_weight(self, editions_weight=None):
        ...
        #print(np.amax(self.weight_map))
        maxm = 1
        if np.amax(self.weight_map) > np.amin(self.weight_map)*(-1):
            maxm = np.amax(self.weight_map)
        elif np.amin(self.weight_map) != 0:
            maxm = np.amin(self.weight_map)*(-1)
            


        
        return self.weight_map / maxm

    def add_weight(self, image):
        self.weight_map = np.add(self.weight_map, image.ravel()*self.positive_weight_multiplier)
...
plot_images(list(a.get_normalised_weight().reshape(28,28) for a in numcats.values())[0:9], list(numcats.keys())[0:9])

for numcat in numcats.values():
    numcat.positive_weight_multiplier = 5
    numcat.negative_weight_multiplier = 5
    

for b in range(3):
    for i, training_image in enumerate(train_X[0:negative_weight_iterations]):
        if train_y[i] not in numcats:
            numcats[train_y[i]] = NumCat(train_y[i])
